...
#define DATA_PIN  D7
#define CS_PIN    D8
byte intensity = 5;                // Led matris parlaklığı 0-15 arası, 0 en düşük
...
void ICACHE_RAM_ATTR TimerHandler(void)
{
  ...
  if (setalarm) { //eğer alarm devredeyse
    ...
    if (sayac == alarmduration || buton==true) { //süre dolduysa veya butona basıldıysa alarmı durduruyoruz
      alarmdurdur();
      sayac = 0;
      buton=false;
    }
...
//EPROM'dan alarm ile ilgili bilgileri okuyoruz.
  ...
  delay(100);
  if (alarmHour2==255){alarmHour2=0;}
  
  alarmMinute2 = EEPROM.read(106); //alarm dakikası 2
  delay(100);
  if (alarmMinute2==255){alarmMinute2=0;}
...
void loop()
{
  ...
  if (P.displayAnimate()) //LED Matrix animasyonlarımız bu satır ve displayReset komutu arasında oynar
  {
    ...
    ////////buton ile alarm susturma
    buton=digitalRead(D0); //HIGH gelirse true yapacak ve kesme içinde alarm durdurulacak
    delay(20);//basılmayı daha düzgün algılaması için
    
    
    //gece düşük parlaklıkla yanacak gündüz yüksek
    if (actualHour > 19 && actualHour < 7){
      P.setIntensity(1);
      } 
...
void handleMainPage() {
  ...
  if (alarmdurum==1){acik="checked=\"checked\"";}
  else {kapali="checked=\"checked\"";} 
  if (alarmdurum2==1){acik2="checked=\"checked\"";}
  else {kapali2="checked=\"checked\"";} 
  ...
  if (alarmdurum==1){ //eğer alarm özelliği aktif ise
    webPage += "Alarm-1<br>";
    if(alarmHour<10){ alarmHour_1="0"+String(alarmHour);}
    else {alarmHour_1=String(alarmHour);}
    if(alarmMinute<10){alarmMinute_1="0"+String(alarmMinute);}
    else{alarmMinute_1=String(alarmMinute);}
  ...
  if (alarmdurum2==1){ //eğer alarm özelliği aktif ise
    if(alarmHour2<10){ alarmHour_2="0"+String(alarmHour2);}
    else {alarmHour_2=String(alarmHour2);}
    if(alarmMinute2<10){alarmMinute_2="0"+String(alarmMinute2);}
    else{alarmMinute_2=String(alarmMinute2);}
...
void handleSetCommand() {
  ...
  if (server.args() == 0) { //hiç argüman gelmemişse
    ...
    if (server.argName(0) == "alarmduration") { //alarmduration isimli gelen veriyi al ve Integer'a çevirip programdaki değişkeni güncelle.
      ...
      if (alarmduration==0 || alarmduration>=60){alarmduration=15;}
    ...
    if (server.argName(4) == "alarmdakika2") {
      ...
      if (alarmMinute2>=60){alarmMinute2=0;}
